<script>
    ...
    <!-- Indicador de scroll horizontal para telas pequenas  -->
    <HelperScroll classes="text-auxiliar-primeira dark:text-branco" />
...
{:then res}
    ...
    >
        {#each res.data.posts.nodes as post, i}
            <CardBlogSegundo
                data={post.date}
                imgDestaque={post.featuredImage.node.hasOwnProperty("guid")
                    ? post.featuredImage.node.guid
                    : ""}
                etiqueta={post.terms.nodes.length > 0
                    ? post.terms.nodes[0].name
                    : ""}
                titulo={post.title}
    ...
    <!-- botões de navegação -->
    <div class="nlf-container nlf-container-filho flex max-md:flex-col max-md:gap-3 justify-between">
        {#if !(res.data.posts.pageInfo.hasPreviousPage)}
        <button
            class="cursor-pointer font-bold rounded-xl text-center max-md:w-full md:w-max nlf-grow px-10 py-5 bg-principal-primeira/20 text-branco">{"<<"}</button
        >
        {/if}
        {#if res.data.posts.pageInfo.hasPreviousPage}
            <button
                on:click={previousPage(res.data.posts.pageInfo.startCursor)}
                class="cursor-pointer font-bold rounded-xl text-center max-md:w-full md:w-max nlf-grow px-10 py-5 bg-principal-primeira text-branco">{"<<"}</button
            >
            <button on:click={homePage} class="cursor-pointer font-bold rounded-xl text-center max-md:w-full md:w-max nlf-grow px-10 py-5 bg-principal-primeira text-branco"> Início </button>
        {/if}
